...
print("9. the model gets a manifest it can drill into, not a chosen subset")
import json as _json
size = len(_json.dumps(res, ensure_ascii=False))
chk(f"handoff is {size} chars, under the 4000 cap", size < 4000, str(size))
chk("headline topics carried inline", len(res["topics"]) == len(res_full_topics), str(res["topics"]))
first = res["topics"][0]
